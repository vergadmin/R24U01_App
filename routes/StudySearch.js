...
async function searchForCT(req, res, next) {
  ...
  .then(response => {
      ...
      // console.log(studies)
      let list = []
      if (studies.length === 0) return list;
  ...
  // A random 6th trial so people don't always get the same results (?)
  // They are default ranked by relevance, but two identical people will always get the same results, so add a rando.
  let randomInt = Math.floor(Math.random() * (20 - 5 + 1) + 5);
